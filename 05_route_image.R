...
trips <- dt %>%
  ...
  select(route_id = scheduleId,
         ...
         trip) %>%
  unnest(trip) %>%
  mutate(route_id = gsub("_", " ", .$route_id),
         route_id = toupper(route_id),
...
# Merge coordinates back with shapes for labeling
shapes_with_coords <- cbind(shapes, coordinates)

# Create ggplot
sp <- ggplot(data = shapes_with_coords) +
  geom_sf() +
  geom_text(aes(x = lon, y = lat, label = paste(round(lon, 4), round(lat, 4))),
            size = 1.5, check_overlap = TRUE) +
  theme_void() +
  theme(plot.background = element_rect(fill = "white", colour = "transparent"))

# Save route image
ggsave("figs/bandungroutes_with_coords.png", plot = sp, width = 1.5*700, height = 1.5*450,
